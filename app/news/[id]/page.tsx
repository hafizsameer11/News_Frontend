...
export async function generateMetadata({
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (newsResponse.success && newsResponse.data && newsResponse.data.slug) {
        const seoResponse = await fetch(`${API_CONFIG.BASE_URL}/seo/news/${newsResponse.data.slug}`, {
          next: { revalidate: 3600 },
        });
        if (seoResponse.ok) {
  ...
  // Fallback metadata
  return {
    title: "News Article | NEWS NEXT",
    description: "Read the latest news article on NEWS NEXT",
  };
}
...
export default async function NewsDetailPage({
  ...
  try {
    ...
    if (news) {
      ...
      if (news.slug) {
        try {
          const response = await fetch(`${API_CONFIG.BASE_URL}/seo/news/${news.slug}/structured-data`, {
      ...
      if (news.category?.id) {
        try {
          ...
          relatedNews =
            relatedData?.data?.news?.filter((n) => n.id !== news.id).slice(0, 3) || [];
        } catch (error) {
